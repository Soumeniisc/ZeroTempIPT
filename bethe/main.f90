...
!	READ PAR.DAT INPUT FILE
	...
	allocate(ep_fB(Norbs),stat=info)
 	...
 	close(13)
        open(unit=13,file='orb_energyA.dat',status='unknown')
         do io=1,Norbs
        read(13,*) ep_f(io)
        end do
	open(unit=13,file='orb_energyB.dat',status='unknown')
         do io=1,Norbs
        read(13,*) ep_fB(io)
        end do
        close(13)
	open(unit=13,file='sub_lattice.dat',status='unknown')
        read(13,*) sub_lattice
        close(13)
...
!	CREATE THE FREQUENCY GRID
	call makegrid		    ! Defined in the makegrid.f file
        ...
        infot=infot+info
	allocate(sigmaB(Norbs,-Nm:Nm),stat=info) ! added by soumen
        infot=infot+info  ! added by Soumen
...
!       Guess self-energy
	   ...
	   sigmaB(j,i)=sum1 !+U**2*alpha**3/(4.d0*(w(i)+ii*eta)) added by soumen
           end do
         end do
          if((idos==1).or.(idos==2))then
           !call findgf()
	    call findgf_soumen() ! added by soumen instaed of "findgf()"
          else
           call ksummation()
          end if
	...
	if(task_id==0) write(*,*)'start from old data'
        ...
        !call readinput()
	call readinput_soumen() ! added by soumen instaed of "readinput()"
...
!  calculate first and third term in the numerator of B 
             B_num2=0.d0
             do io=1,Norbs
          if(sub_lattice==0) then! A sub lattice ! added by soumen
          B_num2(io)=(mu0-mu_c+ep_f(io)-Hart(io))/denm(io)    
	  else 
	   B_num2(io)=(mu0-mu_c+ep_fB(io)-Hart(io))/denm(io)    
	  end if       
            end do
!****************************************************
 ...
 12	  close(24)
	  ...
	  do i=-N,N
	    ...
	    j=locate(N1,wl,1,N1,wi)
          ...
          N1=0
 	  i=1
	  open(unit=25,file='sig1downA.dat',status='unknown')
...
!          
 ...
 ! reading B sublattice self energy
         j=0
         wl=0.d0
         sigl=0.d0        
         r1=0.d0
         ...
         N1=0
	  i=1
	  open(unit=124,file='sig1upB.dat',status='unknown')
 111	  read(124,*,end=112) r1,r2,r3
 	  wl(i)=r1
	  sigl(i)=dcmplx(r3,r2)
	  i=i+1
	  goto 111
 112	  close(124)
	  ...
	  do i=-N,N
	    wi=w(i)
	    j=locate(N1,wl,1,N1,wi)
	      sigmaB(1,i)=sigl(j)+(sigl(j+1)-sigl(j))* &
     		        ((wi-wl(j))/(wl(j+1)-wl(j)))
     	  end do
          ...
          N1=0
 	  i=1
	  open(unit=125,file='sig1downB.dat',status='unknown')
 113	  read(125,*,end=114) r1,r2,r3
 	  wl(i)=r1
	  sigl(i)=dcmplx(r3,r2)
	  i=i+1
	  goto 113
 114	  close(125)
	  ...
	  do i=-N,N
	    wi=w(i)
	    j=locate(N1,wl,1,N1,wi)
	      sigmaB(2,i)=sigl(j)+(sigl(j+1)-sigl(j))* &
     		        ((wi-wl(j))/(wl(j+1)-wl(j)))
     	  end do
!          
...
!!********************************************************
!!********************************************************
subroutine findgf_soumen()
        use Global
        implicit none
        complex*16 sem,gauss,gammaA, gammaB, gendos
        integer io,i,jo,pw,ie,wo,Nenergy,info
	...
	Nenergy = int(4.0/de)
	!write the dos here
        allocate(energy(Nenergy),stat=info)
	do wo = 1,Nenergy
		energy(wo)=-1.99 + de*(wo-1)
	end do

!	if (benchmark == 1) then
!	open(unit=237,file='bethedos.dat',status='unknown')
!	do wo = 1,Nenergy
!	 write(237,'(f12.6,e25.12)')energy(wo), sqrt(4.0 - energy(wo)*energy(wo))/(2*pi)
!	end do
!        close(237)
!	end if !if (benchmark == 1)
         ...
         do io=1,Norbs
          do i=-N,N
            z1=w(i)+ii*eta
            gammaA=z1+mu_c-ep_f(io)-sigma(io,i)
	    gammaB=z1+mu_c-ep_fB(io)-sigmaB(io,i)
          	! do for dos integration
            Gf(io,i) = 0.0
	    do wo = 1, Nenergy
			Gf(io,i)= Gf(io,i) + de*gammaB*sqrt(4.0 - energy(wo)*energy(wo))/((gammaA*gammaB - energy(wo)*energy(wo))*2*pi )
            end do ! w0
          end do  !i
      end do !io
	    ...
	    gammaB=z1+mu_c-ep_fB(io)-sigmaB(io,i)
            ...
            Gf(io,i) = 0.0
	    do wo = 1, Nenergy
			Gf(io,i)= Gf(io,i) + de*gammaA*sqrt(4.0 - energy(wo)*energy(wo))/((gammaA*gammaB - energy(wo)*energy(wo))*2*pi )
            end do ! wo
          end do !i
      end do   !io
